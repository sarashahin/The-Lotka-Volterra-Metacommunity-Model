...
from config import (
    TARGET_RICHNESS,  NUM_CONSUMERS, TOTAL_SPECIES, BODY_MASS, INV, MORTALITY_RATE,
    RANDOM_SEED, TMAX, STEP_SIZE, N_RECORDS, RECORDING_STEP_SIZE,
    CONNECTANCE, INTERACTION_STRENGTH, CONNECTANCE, INTERACTION_STRENGTH, ATTACK_RATE, CONVERSION_EFFICIENCY, RESPIRATION_RATE
)
...
from analysis import (
    ...
    save_model_output,
    mav, bav, mean_se,
    plot_species_trajectories_psd,
    plot_species_trajectories_psd2,
    plot_consumer_resource_comparison
)
...
def main():
    ...
    # 1) S_total includes resources + consumers
    S_total = TOTAL_SPECIES
    
    # 2) random seed
    np.random.seed(RANDOM_SEED)

    # 3) resource growth rates
    r_resources = np.random.normal(loc=1.0, scale=0.1, size=TARGET_RICHNESS)
    
    # 4) consumer growth rates = -RESPIRATION_RATE
    r_consumer = np.array([-RESPIRATION_RATE] * NUM_CONSUMERS)
    r = np.concatenate([r_resources, r_consumer])

    # 5) build adjacency for resource-resource
    adjacency = (np.random.rand(TARGET_RICHNESS, TARGET_RICHNESS) < CONNECTANCE).astype(float)
    C_resources = INTERACTION_STRENGTH * adjacency
    np.fill_diagonal(C_resources, 1.0)

    # 6) put resource-resource block in big matrix
    C = np.zeros((S_total, S_total))
    C[:TARGET_RICHNESS, :TARGET_RICHNESS] = C_resources

    # 7) resource-consumer = +ATTACK_RATE
    C[:TARGET_RICHNESS, TARGET_RICHNESS:] = ATTACK_RATE

    # 8) consumer-resource = -ATTACK_RATE * CONVERSION_EFFICIENCY
    C[TARGET_RICHNESS:, :TARGET_RICHNESS] = - ATTACK_RATE * CONVERSION_EFFICIENCY

    # 9) consumer-consumer = 0
    #    (remains zero from initialization)

    logger.info(f"Initial growth rates (r) and interaction matrix (C) set for S_total={S_total} species.")

    # Create a Boolean array indicating which species are consumers (last NUM_CONSUMERS species)
    is_consumer = np.array([False] * TARGET_RICHNESS + [True] * NUM_CONSUMERS)

    # RUN IBM Model
    logger.info("Running IBM Model...")
    ibm_model = IBMModel(r=r, C=C, nsteps=TMAX, record_step=RECORDING_STEP_SIZE, seed=RANDOM_SEED, is_consumer=is_consumer)
    ibm_trajectory = ibm_model.run()

    # RUN PSD Model
    logger.info("Running PSD Model...")
    psd_model = PSDModel(r=r, C=C, nsteps=TMAX, record_step=RECORDING_STEP_SIZE, seed=RANDOM_SEED, is_consumer=is_consumer)
    ...
    psd2_model = PSD2Model(r=r, C=C, tmax=TMAX, record_step=RECORDING_STEP_SIZE, seed=RANDOM_SEED, is_consumer=is_consumer)
    ...
    ode_model = ODEModel(r=r, C=C, tmax=TMAX, record_step=RECORDING_STEP_SIZE, seed=RANDOM_SEED, is_consumer=is_consumer)
    ode_times, ode_trajectory = ode_model.run()

    # Basic analysis and plotting
    logger.info("Generating basic analysis plots...")
    
    # Create line plots for PSD and PSD2 specifically:
    plot_species_trajectories_psd(psd_trajectory, filename="PSD_Species_Trajectories.png")
    plot_species_trajectories_psd2(psd2_trajectory, filename="PSD2_Species_Trajectories.png")
    ...
    # Compare average biomass of resources vs consumers 
    plot_consumer_resource_comparison(ibm_trajectory, is_consumer, filename="Consumer_vs_Resource.png")
