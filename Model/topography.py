...
class Topography:
    def __init__(self, no_nodes, skVec, scVec,lattice_height, lattice_width, phi, envVar,
                  var_e, randGraph, gabriel, T_int, min_env=None, range_env=None, env_mat=None, network_file=None, sc_file=None, env_file=None, consArea_bin=None, consArea_multiplicative=False):
        # Parameters
    ...
    def gen_network(self):
        ...
        if self.network_file:  # Import network from the provided file
          ...
          try:
              # Assuming the network is stored in ASCII format with two columns (x, y coordinates)
              self.network = np.loadtxt(self.network_file)
              self.no_nodes = self.network.shape[0]
              logger.debug(f"Imported network with shape: {self.network.shape}")
          except Exception as e:
              logger.error(f"Failed to load network from ASCII file: {e}")
              raise
        else:  # Generate network
            if self.randGraph:  # Random graph as in topography.cpp
        ...
        logger.debug(f"Network generated or imported with shape: {self.network.shape}")
    ...
    def gen_environment(self):
        ...
        if self.env_file:  # Import environmental data from the provided ASCII file
            ...
            try:
                ...
                self.env_mat = np.loadtxt(self.env_file)
                logger.debug(f"Imported environment matrix with shape: {self.env_mat.shape}")
            except Exception as e:
            ...
            self.env_mat = np.zeros((self.envVar, self.no_nodes))
            ...
            self.range_env = np.ptp(self.env_mat, axis=1)
            self.min_env = np.min(self.env_mat, axis=1)
    ...
    def gen_temp_grad(self):
        ...
        self.env_mat = self.T_int - T_grad * self.network[:, 0]
        logger.debug(f"Generated temperature gradient with shape: {self.env_mat.shape}")
    ...
    def gen_landscape(self, net_imported=None):
        ...
        if self.envVar != 0 and not self.env_file and (self.env_mat is None or self.env_mat.size == 0):
...
if __name__ == "__main__":
    topo = Topography(
        ...
        T_int=25.0,
        network_file="/content/drive/MyDrive/Simulation_data/normDiscrTraj04_experiment/2021-3-1/2021-3-1_normDiscrTraj04(10000)1network0.mat",  # Example ASCII file for network
        env_file="/content/drive/MyDrive/Simulation_data/normDiscrTraj04_experiment/2021-3-1/2021-3-1_normDiscrTraj04(10000)1rMat0.mat",  # Example file for environment
        sc_file="/content/drive/MyDrive/Simulation_data/normDiscrTraj04_experiment/2021-3-1/2021-3-1_normDiscrTraj04(10000)1S0.mat"
    )
