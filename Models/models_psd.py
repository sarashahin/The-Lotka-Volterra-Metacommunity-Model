...
class PSDMultiPatchModel:
    def __init__(self, r, C, nsteps=None, record_step=None, seed=123, hetero_strength=0.05):
        ...
        # NEW: record the *actual flux* (not just “flux / B”) at each record:
        self.dispersal_flux_traj = np.full(
            (self.nrecords, NUM_PATCHES_Y, NUM_PATCHES_X, self.S), np.nan, dtype=float
        )
    ...
    def run(self):
        ...
        for s in range(self.nsteps):
            ...
            if (s + 1) % self.record_step == 0:
                ...
                # Also store the flux in dispersal_flux_traj:
        ...
        return (self.trajectory, self.wait_trajectory,
                ...
                self.dispersal_term_traj,
                self.dispersal_flux_traj,  # <--- Return the flux array
