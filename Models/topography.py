...
logger.setLevel(logging.DEBUG)  # Enable DEBUG for logs
...
class Topography:
    def __init__(self, no_nodes, skVec, scVec,lattice_height, lattice_width, phi, envVar,
                  var_e, randGraph, gabriel, T_int, min_env=None, range_env=None, envMat=None, network_file=None, sc_file=None, env_file=None, consArea_bin=None, consArea_multiplicative=False, network_file_type='ascii', env_file_type='ascii', sc_file_type='ascii'):
        ...
        # File Type Flags
        self.network_file_type = network_file_type
        self.env_file_type = env_file_type
        self.sc_file_type = sc_file_type
        ...
        if scVec is not None:
            self.scVec = scVec
            self.scVec_prime = np.sqrt(scVec)  
        else:
            # Assign default scaling vectors
        ...
        if range_env is not None:
            ...
            # Range of environmental variables across nodes
        ...
        if min_env is not None:
            ...
            self.min_env = np.zeros(self.envVar)  # All minimums set to 0
    ...
    def gen_network(self):
        ...
        if self.network_file:  # Import network from the provided file
            ...
            try:
                ...
                elif self.network_file_type == 'ascii':
                    self.network = np.loadtxt(self.network_file, dtype=float, comments='#')
                else:
                    raise ValueError(f"Unsupported network_file_type: {self.network_file_type}")
    ...
    def gen_landscape(self, net_imported=None):
        ...
        if net_imported is None or net_imported.shape[0] == 0:
            try:
                self.gen_network()  # Generate network
            except Exception as e:
        ...
        if self.sc_file:
            try:
                ...
                if self.sc_file_type == 'mat':
                    scaling_matrix = loadmat(self.sc_file)
                    scaling_vectors = scaling_matrix.get('scaling_vectors')  # Adjust key as per your file
                    if scaling_vectors is None:
                        raise ValueError("'scaling_vectors' key not found in MAT file.")
                    scVec = scaling_vectors.flatten()
                    scVec_prime = np.sqrt(scVec)
                elif self.sc_file_type == 'ascii':
                    ...
                    raise ValueError(f"Unsupported sc_file_type: {self.sc_file_type}")
                ...
                if len(scVec) != self.no_nodes:
                    # Adjust scaling vectors to match no_nodes=32
                    if len(scVec) > self.no_nodes:
                        logger.warning(f"Scaling vector length ({len(scVec)}) exceeds node count ({self.no_nodes}). Truncating.")
                        scVec = scVec[:self.no_nodes]
                        scVec_prime = scVec_prime[:self.no_nodes]
                    elif len(scVec) < self.no_nodes:
                        logger.warning(f"Scaling vector length ({len(scVec)}) is less than node count ({self.no_nodes}). Padding with ones.")
                        scVec = np.pad(scVec, (0, self.no_nodes - len(scVec)), 'constant', constant_values=1.0)
                        scVec_prime = np.sqrt(scVec)
...
if __name__ == "__main__":
        ...
        # Initialize scaling vector with 1 entries
    scVec = np.linspace(0.05, 0.15, 32) 

    # Initialize species scaling vectors (skVec) with envVar=2
    skVec = np.array([0.1, 0.2])  # Must match envVar=2
    ...
    plt.scatter(topo.network[:, 0], topo.network[:, 1], c='blue')
    ...
    if topo.envMat is not None:
        plt.figure(figsize=(10, 4))
        for i in range(topo.envVar):
            plt.subplot(1, topo.envVar, i+1)
            plt.scatter(range(topo.no_nodes), topo.envMat[i, :], c='green')
            plt.title(f'Environmental Variable {i+1}')
            plt.xlabel('Node')
            plt.ylabel('Value')
        plt.tight_layout()
        plt.show()
