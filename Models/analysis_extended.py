...
#############################
# One-Time-Step Visualization for PSD Biomass
#############################

def interactive_one_timestep_psd(model_data, timestep=None):
    """
    Creates an interactive bar plot showing the biomass distribution of all species
    from the PSD model at a single time step. The PSD trajectory is assumed to be in log-scale.
    If 'timestep' is not provided, the last recorded time step is used.
    """
    psd_log = model_data["PSD"]  # PSD trajectory stored in log-scale
    n_records, n_species = psd_log.shape
    if timestep is None:
        timestep = n_records - 1
    elif timestep < 0 or timestep >= n_records:
        logger.warning(f"Provided timestep {timestep} is out of bounds. Using last time step.")
        timestep = n_records - 1
    # Convert log biomass to biomass via exponentiation.
    biomass = np.exp(psd_log[timestep, :])
    species_ids = np.arange(n_species)
    df = pd.DataFrame({"Species ID": species_ids, "Biomass": biomass})
    fig = px.bar(df, x="Species ID", y="Biomass",
                 title=f"PSD Model: Biomass Distribution at Time Step {timestep}",
                 labels={"Species ID": "Species ID", "Biomass": "Biomass"})
    output_file = os.path.join(OUTPUT_FOLDER, f"Interactive_PSD_Biomass_TimeStep_{timestep}.html")
    fig.write_html(output_file)
    fig.show()
    logger.info(f"Saved interactive one-timestep PSD biomass plot as {output_file}.")
