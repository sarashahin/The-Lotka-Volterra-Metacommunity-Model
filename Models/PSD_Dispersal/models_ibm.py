...
Local dynamics are updated in each patch, then dispersal is applied over the entire grid.
Additionally, a spatial diagnostic (dispersal flux) is recorded at each recording step.

>> NEW: To introduce spatial heterogeneity, each patch now gets its own intrinsic growth rate
   vector (r_field) that is derived from the global r with a small random perturbation.
"""
...
class IBMMultiPatchModel:
    def __init__(self, r, C, nsteps=None, record_step=None, seed=123, hetero_strength=0.05):
        ...
        """
        self.global_r = r  # store global r
        self.C = C
        self.S = len(r)
        ...
        # For each patch, perturb the global r by adding a small random noise.
        self.r_field = np.zeros((NUM_PATCHES_Y, NUM_PATCHES_X, self.S))
        for i in range(NUM_PATCHES_Y):
            for j in range(NUM_PATCHES_X):
                # Each patch gets: global r + hetero_strength * normal_noise
        ...
        initial_count = int(init_biomass / BODY_MASS)
        # State array: (NUM_PATCHES_Y, NUM_PATCHES_X, S)
        self.N = np.full((NUM_PATCHES_Y, NUM_PATCHES_X, self.S), initial_count, dtype=int)
    ...
    def run(self):
        logger.info("Starting IBM multi-patch simulation with dispersal and spatial heterogeneity...")
        record_idx = 0
        for s in range(self.nsteps):
            ...
            for i in range(NUM_PATCHES_Y):
                for j in range(NUM_PATCHES_X):
                    ...
                    # --- CHANGE: Use the patch-specific growth rates from r_field instead of global r ---
            ...
            # --- Dispersal Update ---
            # Convert entire state to biomass.
            B_all = self.N * BODY_MASS
            # Compute dispersal flux over the whole grid.
            flux = compute_dispersal(B_all)
            # Record the dispersal flux diagnostic.
