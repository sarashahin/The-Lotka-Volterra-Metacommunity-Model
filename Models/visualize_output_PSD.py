...
def plot_time_series(data, outdir="."):
    ...
    # We can define total species as producers + consumers
    ...
    # -- Panel 1: producers vs. consumers
    fig.add_trace(
    ...
    # -- Panel 4: producers vs consumers (scatter)
    fig.add_trace(
...
def plot_psd_metrics(data, outdir="."):
    ...
    # 1) Ensure each array has the same first dimension (num_records)
    #    e.g., if you used a "reconcile_array_lengths" step, that should be done prior to calling this function.
    #    We'll just do a basic check here:
    ...
    if not all([
        ...
        return  # One or more mismatch; abort

    # 2) If any arrays are 3D (num_records, max_species, nodes),
    #    reduce them to 2D by averaging across the node dimension (axis=2).
    #    e.g. if PSD_states is (150, 127, 25), do PSD_states = PSD_states.mean(axis=2).
    def ensure_2d(arr, arr_name):
        if arr.ndim == 3:
            ...
            # As fallback, we could do a .mean(axis=tuple(range(2, arr.ndim))) if >3D
            # For simplicity:
    ...
    # 3) Now each is shape (num_records, max_species).
    #    We'll define aggregated metrics across species (like averages).
    # Example: proportion of species in state=1
    ...
    # 4) Build a subplot figure for the first 4 metrics
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=(
            "Proportion of Species in Waiting States",
    ...
    # panel 4: average establishment prob
    fig.add_trace(
    ...
    #
    #  [OPTION A] Single-line average invasion rate across species
    #
    # (If you only want one line, uncomment this block)
    #
    # fig2 = px.line(
    #     x=iterations,
    #     y=avg_inv_rate,  # shape (num_records,)
    #     labels={"x": "Iterations", "y": "Average Invasion Rate"},
    #     title="Average Invasion Rate Over Iterations",
    # )
    # fig2.update_layout(template="plotly_dark", width=1000, height=600)
    # outpath2 = os.path.join(outdir, "average_invasion_rate.html")
    # fig2.write_html(outpath2)
    # print(f"Saved average invasion rate figure to {outpath2}")

    #
    # [OPTION B] Multi-line plot of invasion rate per species
    # 
    # We'll assume 'inv_rate' is now shape (num_records, max_species)
    # after the ensure_2d(...) step.
    # Each column => 1 species across time, so we do a multi-line plot:
    #
...
def plot_producer_consumer_bar(data, outdir=".", barmode="group"):
    ...
    # Build a DataFrame for easy plotting with Plotly Express
    df = pd.DataFrame({
...
def plot_psd_states_bar(data, outdir="."):
    ...
    if PSD_states.ndim == 3:
        # e.g., shape (num_iterations, num_species, 2)
    ...
    # Optional: specify custom colors
    color_map = {
...
def plot_histogram_of_metric(
    ...
    arr = data[metric]  # could be 2D or 3D
    iterations = data["iteration"]
    num_iterations = len(iterations)
    
    # 2) If arr is 3D, decide how to reduce the node dimension:
    ...
    if iteration_index is None:
        ...
        # If arr is 3D with node_aggregation="none", we have shape => (num_iterations, max_species, nodes).
    ...
    # 4) Filter out invalid or infinite values
