...
class SimulationRunner(abc.ABC):
    ...
    def _parse_cli(self, argv):
        ...
        parser.add_argument('--save-every-rounds', type=int, default=50)
    ...
    def post_process_and_save(self, results):
        ...
        output_payload = {
            "gamma": np.int64(len(r_final)), "r_base": r_final.astype(np.float32),
            ...
            "P_t": P_t, "B_last": B_last, "runtime_s": np.float32(results['runtime_s']),
            "detection_threshold": np.float32(detection_thr_count),
